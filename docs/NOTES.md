...
- **User namespaces** - Steam's pressure-vessel runtime requires `user.max_user_namespaces`, which Talos defaults to `0`. Enabled via a per-machine Omni config patch (`520-cm-<machine-uuid>-user-namespaces`, `user.max_user_namespaces: "11255"`) on every node; the template lives in the gitignored `.omni/cluster1/self-hosted/cluster-template.yaml`, not in the reconciled tree. The patch is keyed by machine UUID, so re-enrolling a node under a new UUID (nv-01 after its board swap) silently drops it: Steam then exits with code 71 in a 2s restart loop and `console-linux.txt` says `Steam now requires user namespaces to be enabled`. Check with `cat /proc/sys/user/max_user_namespaces` from any pod on the node; `omnictl cluster template diff` shows the missing ConfigPatch. Also used by the `auth` workload, which runs with `hostUsers: false` so its containers' root maps to an unprivileged host UID.
...
- **DRM render-node override** - `wolfConfig.runtimeVariables.renderNode` defaults to `/dev/dri/renderD129`, but with two 5090s the device plugin may allocate the Wolf sidecar the other card, and CDI injects only that card's node (`renderD130`) with host numbering (`renderD128` does not exist inside the container). With a node that is not present Wolf fails GPU vendor detection, falls back to software x264/x265, and `waylanddisplaysrc` cannot open the node, so the Wayland display never starts: the app container loops on `Waiting for wayland display...` and Moonlight connects (RTSP/audio work) but shows no video. The root entrypoint ConfigMap (`wolf-entrypoint.yaml`) therefore checks `WOLF_RENDER_NODE` at boot and, if absent, exports the injected NVIDIA render node (vendor `0x10de` in `/sys/class/drm`) before starting Wolf. Wolf reads the variable once at config load and every session app inherits it. With the sidecar now privileged (below) every host node is visible, so `renderD129` is always present and pins the whole session (compositor, NVENC, Sway's dmabuf device, Vulkan device 0) to the slot-1 card; the autodetect stays as the unprivileged fallback.
- **Privileged Wolf sidecar (NVENC on a multi-GPU host)** - `libnvidia-encode` opens an encode session by asking the Resource Manager for the attached GPU IDs (all host GPUs, even in a 1-GPU container) and then opening each `/dev/nvidiaN` for peer init. With only the allocated card's node injected it returns `NV_ENC_ERR_UNSUPPORTED_DEVICE`, GStreamer logs `nvh265encoder ... Failed to open session` at plugin load, nvcodec registers no encoders, and Wolf silently falls back to software x264/x265 at full stream resolution (driver regression since 570.x, present on 595.71.05; NVIDIA/nvidia-container-toolkit#1249, NVIDIA/k8s-device-plugin#1282). Reproduced with a raw `nvEncOpenEncodeSessionEx` from the wolf image: unprivileged + 1 GPU fails, privileged + 1 GPU registers `nvh264enc`/`nvh265enc`/`nvav1enc`. `sidecarPolicies.wolf.securityContext.privileged: true` in `user.yaml` is the workaround; revisit when the Talos nvidia extension ships a driver with the RM fix (NVIDIA points at 610.x) or by adding an LD_PRELOAD attached-IDs filter to the `shrinedogg/wolf` image.
- **Per-session input isolation / no in-container udevd** - session pods get a pod-private emptyDir at `/dev/input`; the wolf-agent (root + `MKNOD`) creates nodes only for this session's wolf devices (sysfs-resolved major/minor from the per-session `PlugDevice` events) and writes their udev db entries + netlink events. The previous design mounted the host `/dev/input` and ran in-container `systemd-udevd` + `udevadm trigger`: uinput pads are kernel-global with no session identity, so each session enumerated every other session's wolf pads and Steam Input pads (duplicate controllers, cross-input), and the trigger replayed host-wide sysfs back into the pod. Steam Input's own emulated pads are kernel-global **with no creator identity in their uevents** (verified live: joey's new pad's uevent reached stacie's udevd monitor), so they can never be scoped per session — they get no node by design, and games must read the wolf pad directly: set the game's controller override to **Disable Steam Input** (BPM: game > gear icon > Controller Options), or the game sees no controller (menus still work, since BPM reads the wolf pad).
