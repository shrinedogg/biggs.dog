...
- **CUDA graphs + context on 32 GB** — runs with `fp8` KV cache, `--max-num-seqs 16` / `--max-cudagraph-capture-size 16`, **`--max-model-len 131072` (128K)**, and `--gpu-memory-utilization 0.95`. The model's **hybrid sliding-window (1024) + global attention** keeps most layers' KV bounded, so long context is cheap — 128K fits comfortably where the old full-attention 27B capped at 65K. The CUDA driver context still caps usable util near ~0.984.
