...
- **CUDA graphs + context on 32 GB** — runs with `fp8` KV cache, `--max-num-seqs 16` / `--max-cudagraph-capture-size 16`, **`--max-model-len 262144` (262K, the model's native max)**, and `--gpu-memory-utilization 0.95` (headroom for the larger ~19B-on-disk weights; raise toward 0.98 once stable). The model's **Mamba-hybrid attention** keeps most layers' KV bounded, so even the full window is cheap. Text-only `--limit-mm-per-prompt={"image":0,"video":0}` skips the vision encoder on this multimodal checkpoint.
