...
- **CUDA graphs on 32 GB** — the 27B model nearly fills the card, so fitting CUDA-graph capture required `fp8` KV cache, a small `--max-num-seqs 8` / `--max-cudagraph-capture-size 8` (kagent is single-user), a 64K context (`--max-model-len 65536`), and `--gpu-memory-utilization 0.9833` — safe to push that high because `nv-01` is a headless Talos node with a dedicated GPU (no OS/display VRAM), though the CUDA driver context caps util at ~0.984. vLLM reserves graph-capture memory up front, then fills the KV pool from the rest.
...
- **Long-term memory disabled** — `vectorEnabled: false` in the kagent HelmRelease. With pgvector memory on, kagent's memory subsystem inflates every request's context to the model's full window (`max_model_len + 1` tokens), overflowing heavy-schema agents like `flux-agent` on the 65536-token model. Within-session conversation memory still works; only semantic recall across separate sessions is lost — negligible for the one-shot operational tasks these agents handle via MCP. Re-enabling cross-session memory needs a smaller-weight, longer-context model that leaves KV-cache headroom for the inflated context.
