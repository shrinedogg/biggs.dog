...
| `nvidia-gpu-tuning` (DaemonSet) | Privileged DaemonSet pinned to `nv-01` running `nvidia-smi -pm 1 -lgc 0,2800 -pl 450` to cap the RTX 5090's boost clock (stock max 3105 MHz; Xid 109 CTX SWITCH TIMEOUT mitigation) and board power (stock TDP 600 W → 450 W cap); re-asserted every 5 min. Consumes one of the 4 GPU time-slices to trigger CDI injection of `nvidia-smi`. |
